...
leaflet(acag3) %>% addTiles() %>%
  addCircleMarkers(
    color = ~pal(X2000_p_SO4.x),
    stroke = FALSE, fillOpacity = 0.5
  ) %>% addLegend("bottomright", pal = pal, values = ~X2000_p_SO4.x,
...
pal <-  colorNumeric("magma", domain = 1:10)

leaflet(no23) %>% addTiles() %>%
  addCircleMarkers(radius=3,
    color = ~pal(X2008),
    stroke = FALSE, fillOpacity = 0.5
  ) %>% addLegend("bottomright", pal = pal, values = ~X2008,
                  title = "Est. NO2 Concentration 2008",
                  labFormat = labelFormat(prefix = "$"),
                  opacity = 1
  )
